...
def selectWeakest(board):
    ...
    for n in range(1,N+1):
        for m in range(1,M+1):
            if (power, -time, (port[0]+port[1]), -port[1], port) > (board[n][m], -recent[n][m], (n+m), -m, [n,m]) and board[n][m] > 0:
...
def attack(board, n, m, k):
    ...
    if laser:
        ...
        while path:
            ...
            recent[attackR][attackC] = k

        #최종 목표 공격
        board[targetR][targetC] -= Power
        recent[targetR][targetC] = k
    ...
    if not laser:
        ...
        for dr, dc in d:
            ...
            if board[neighR][neighC] != 0 and [neighR, neighC] not in neighbor: 
...
for k in range(1,K+1):
    ...
    board = attack(board, weakest[0], weakest[1], k)
    #print("공격:", board)
    
    #4. 포탑 정비 (부서지지 않은 포탑 중 공격과 무관한 탑 공격력 + 1)
    for n in range(1,N+1):
        for m in range(1,M+1):
            if recent[n][m] < k and board[n][m] > 0:
                board[n][m] += 1
    #print("recent:",recent)
