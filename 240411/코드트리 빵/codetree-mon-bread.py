...
def closest(targetR, targetC):
    
    minDist = float('inf')
    minR, minC = float('inf'), float('inf')
    for b in basecamp:
        bR, bC = b
        curDist = abs(bR-targetR) + abs(bC-targetC) - 1
        if [bR, bC] not in prohibited:
            if (minDist, minR, minC) > (curDist, bR, bC):
                minDist = curDist
                minR, minC = bR, bC
...
def move(board, t):
    ...
    if t <= m and not toBase[t]:
        baseR, baseC = closest(targetR, targetC)
        #print(baseR, baseC)
